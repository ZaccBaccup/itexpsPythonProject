...
NewJob = JobApplication("Among Us", "Impsoster", 2,5,2025,"N/A",False)
...
with open(fileToRead, 'r') as csvfile:
    ...
    for curRow in csvreader:#adds the row into a JobApplication Object
        try:
            jobApplicationList.append(JobApplication(curRow[0], curRow[1],int(curRow[2]), int(curRow[3]),int(curRow[4]), curRow[5],bool(int(curRow[6]))))
        except IndexError:
            print("Uh oh! This job application only has", len(curRow), "entries when it should have 7!")
